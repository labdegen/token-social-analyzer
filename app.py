...
class SocialCryptoDashboard:
        ...
        **6. LIVE TWEETS:**
        ...
        try:
            ...
            if result and len(result) > 200 and "API key" not in result:
                ...
                return self._parse_comprehensive_analysis_enhanced(result, token_address, symbol)
            else:
                logger.warning(f"GROK API returned insufficient data or API key issue: {result[:100] if result else 'No result'}")
                return self._get_fallback_comprehensive_analysis(symbol, market_data)
    ...
    def _format_expert_analysis_html(self, sections: Dict, symbol: str, raw_text: str = "") -> str:
        """Format expert analysis as HTML with proper headings - extend real content"""
        
        html = f"<h2>🎯 Social Intelligence Report for ${symbol}</h2>"
        
        # Use any available sections with extended real content
        sections_found = False
        
        if sections.get('sentiment'):
            sentiment_content = sections['sentiment']
            # Extend the real content instead of adding placeholder text
            html += f"<h2>📊 Sentiment Analysis</h2><p>{sentiment_content}</p>"
            sections_found = True
        
        if sections.get('influencer'):
            influencer_content = sections['influencer']
            html += f"<h2>👑 Key Account Activity</h2><p>{influencer_content}</p>"
            sections_found = True
        
        if sections.get('risks'):
            risk_content = sections['risks']
            html += f"<h2>⚠️ Risk Factors</h2><p>{risk_content}</p>"
            sections_found = True
        
        if sections.get('trading'):
            trading_content = sections['trading']
            html += f"<h2>📈 Trading Signal</h2><p>{trading_content}</p>"
            sections_found = True
        
        if sections.get('prediction'):
            prediction_content = sections['prediction']
            html += f"<h2>🔮 Market Prediction</h2><p>{prediction_content}</p>"
            sections_found = True
        
        # If we have sections, add live data note
        if sections_found:
            html += f"<h2>📱 Live Data Integration</h2><p>Analysis powered by real-time X/Twitter data with Solana-specific filtering for ${symbol}.</p>"
        ...
        # If no sections found but we have raw text, use that
            ...
            # Clean up the raw text and use it directly
            ...
            if len(sentences) > 3:
                ...
                html += f"<h2>📊 Social Analysis</h2><p>{first_part}</p>"
                html += f"<h2>📱 Live Analysis</h2><p>{second_part}</p>"
            else:
                html += f"<h2>📊 Social Analysis</h2><p>{clean_text}</p>"
        ...
        # Final fallback
            ...
            <p>Connect XAI API key for comprehensive social sentiment analysis with live X/Twitter data, KOL activity tracking, and community sentiment metrics for Solana ${symbol}.</p>
            <h2>📈 Market Insights</h2>
            <p>Advanced analysis includes trending discussions, influencer activity, and social momentum scoring specifically for Solana blockchain ${symbol} token.</p>
            """
        
        return html
    
    def _extract_actual_tweets_improved(self, text: str, symbol: str) -> List[Dict]:
        """Improved tweet extraction for real content - NO FALLBACKS"""
        ...
        for pattern in tweet_patterns:
            ...
            for match in matches:
                if len(match) >= 2:
                    ...
                    if (len(content) > 20 and content_clean not in seen_tweets and 
                        (symbol.lower() in content.lower() or 
                         any(word in content.lower() for word in ['crypto', 'token', 'solana', 'pump', 'moon']))):
        ...
        # Return only real tweets found, max 6
        unique_tweets = []
        seen_content = set()
        for tweet in tweets:
...
def health():
    return jsonify({
        'status': 'healthy',
        'version': '4.3-comprehensive-fixes',
        'timestamp': datetime.now().isoformat(),
        'features': [
